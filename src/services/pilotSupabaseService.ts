...
export async function findBarcode(barcodeValue: string): Promise<BarcodeLookup | null> {
  if (!hasSupabaseEnv || !supabaseClient) {
    ...
    if (!sku) return null;
    const isCarton = sku.barcodeCarton === barcodeValue;
    return {
      sku_id: sku.id,
      ...
      barcode_type: isCarton ? 'carton' : 'sleeve',
      unit_level: isCarton ? 'carton' : 'sleeve',
      quantity_in_base_unit: isCarton ? (sku.sleevesPerCarton ?? 1) : 1
    };
  }

  const { data, error } = await supabaseClient
    .from('barcodes')
    .select('sku_id, barcode_value, barcode_type, skus!inner(sku_code, display_name, category, sleeves_per_carton)')
    .eq('barcode_value', barcodeValue)
    .maybeSingle();

  if (error || !data) return null;
  const sku = (data as any).skus;
  const barcodeType = (data as any).barcode_type as BarcodeLookup['barcode_type'];
  const isCarton = barcodeType === 'carton';
  return {
    sku_id: (data as any).sku_id,
    sku_code: sku.sku_code,
    display_name: sku.display_name,
    category: sku.category,
    barcode_value: (data as any).barcode_value,
    barcode_type: barcodeType,
    unit_level: barcodeType,
    quantity_in_base_unit: isCarton ? (sku.sleeves_per_carton ?? 1) : 1
  };
}
